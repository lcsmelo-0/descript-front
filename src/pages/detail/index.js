...
const Detail = props => {
  ...
  if (workflow && workflow.status === 200) {
    if (userType.isEditor) return <Redirect from={`/detail/${props.match.params.id}`} to="/editor" />
    if (userType.isReviser) return <Redirect from={`/detail/${props.match.params.id}`} to="/reviser" />
  }
  ...
  const handleApprove = event => {
    ...
    let imageId = props.match.params.id
    let workflowId = image.workflow ? image.workflow.id : null
    let description = image.texts ? image.texts[image.texts.length - 1].text : null
    dispatch(
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Col md="12">
                  <FormGroup>
                    ...
                    {image.texts ? (
                      <Input type="textarea" disabled value={image.texts[image.texts.length - 1].text} />
                    ) : (
                      <></>
                    )}
                  </FormGroup>
                ...
                <Col md="12">
                  <FormGroup>
                    ...
                    <Input type="textarea" name="description" id="description" onChange={handleFieldsChange} />
                  </FormGroup>
                </Col>
